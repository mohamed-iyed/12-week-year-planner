...
// vision options
const options = ["personal", "religious", "job", "school", "health", "other"];
// options images
...
export default function Vision() {
  ...
  const { id } = useParams();
  const { journeys, getJourney, addVision, deleteVision } = useAppContext();
  const { startDate, endDate, visions } = getJourney(id);
  ...
  return (
    <>
      ...
      <p className="text-center">
        ...
        <br /> to : {endDate.toDateString()}
      </p>
      <section className="flex-1 pb-8 flex flex-wrap gap-5 bg-gray-50 rounded-md shadow-xl p-2">
        {visions.length === 0 && (
          <article className="p-1 basis-80 border-0 border-r-2 flex flex-col gap-1 rounded-lg">
            <header className="font-light">Exemple :</header>
            ...
            <ul className="pl-4 flex flex-col gap-1">
              <li className="flex gap-1 bg-white w-fit px-2 py-1 rounded-md shadow-md">
                ...
                <p className="">I want become smarter</p>
              </li>
              <li className="flex gap-1 bg-white w-fit px-2 py-1 rounded-md shadow-md">
                <img src={ItemBullet} width="10" height="10" />
                <p className="">....</p>
        ...
        {visions.map((visionSector: any, i: number) => (
        ...
        <article className="p-1 min-w-[250px] flex-auto flex items-center justify-center flex-col gap-1 rounded-lg">
          <form
            ...
            onSubmit={(e: any) => {
              ...
              if (
                sector.trim() === "" ||
                visions.find((v: any) => v.text === sector)
              ) {
...
function VisionSector({ id: visionId, text, ideas }: any) {
  ...
  return (
    <article
      className={`relative p-1 md:min-w-[300px] md:max-w-[360px] max-w-full border-0 sm:border-r-2 flex flex-col gap-1 rounded-lg`}
    >
      <button
        onClick={() => deleteVision(visionId, id)}
        className="absolute right-1 p-1 flex items-center justify-center shadow-md rounded-[50%] bg-white"
      >
